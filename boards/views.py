...
def about(request):
    return HttpResponse(request, "welcome to the about page")


def boards_topics(request, board_id):
    board = get_object_or_404(Board, pk=board_id)
    querrySet = board.topics.order_by('-created_date').annotate(comments=Count('posts'))
    page = request.GET.get('page',1)
    paginator = Paginator(querrySet,20)
    try :
